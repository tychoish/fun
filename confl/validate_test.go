...
func TestValidate_valid_structs(t *testing.T) {
	...
	for tt := range slices.Values(tests) {
		t.Run(tt.name, func(t *testing.T) {
			assert.NotError(t, Validate(tt.cfg))
		})
	}
}
...
func TestAppendRestArgs_all_types(t *testing.T) {
	...
	t.Run("int64", func(t *testing.T) {
		...
		assert.NotError(t, appendRestArgs(fval, []string{"42", "100"}))
		assert.True(t, slices.Equal(s, []int64{42, 100}))
	})

	t.Run("int64 error", func(t *testing.T) {
		var s []int64
		fval := reflect.ValueOf(&s).Elem()
		assert.Error(t, appendRestArgs(fval, []string{"notanint"}))
	})

	t.Run("int32", func(t *testing.T) {
		var s []int32
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"32"}))
		assert.True(t, slices.Equal(s, []int32{32}))
	})
	...
	t.Run("int16", func(t *testing.T) {
		var s []int16
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"16"}))
		assert.True(t, slices.Equal(s, []int16{16}))
	})
	...
	t.Run("int8", func(t *testing.T) {
		var s []int8
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"8"}))
		assert.True(t, slices.Equal(s, []int8{8}))
	})
	...
	t.Run("uint", func(t *testing.T) {
		var s []uint
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"42"}))
		assert.True(t, slices.Equal(s, []uint{42}))
	})

	t.Run("uint error", func(t *testing.T) {
		var s []uint
		fval := reflect.ValueOf(&s).Elem()
		assert.Error(t, appendRestArgs(fval, []string{"notauint"}))
	})

	t.Run("uint64", func(t *testing.T) {
		var s []uint64
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"64"}))
		assert.True(t, slices.Equal(s, []uint64{64}))
	})
	...
	t.Run("uint32", func(t *testing.T) {
		...
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"32"}))
		assert.True(t, slices.Equal(s, []uint32{32}))
	})
	...
	t.Run("uint16", func(t *testing.T) {
		...
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"16"}))
		assert.True(t, slices.Equal(s, []uint16{16}))
	})
	...
	t.Run("uint8", func(t *testing.T) {
		...
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"8"}))
		assert.True(t, slices.Equal(s, []uint8{8}))
	})
	...
	t.Run("float32", func(t *testing.T) {
		var s []float32
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"3.14"}))
		assert.True(t, len(s) == 1 && s[0] == float32(3.14))
	})

	t.Run("float32 error", func(t *testing.T) {
		var s []float32
		fval := reflect.ValueOf(&s).Elem()
		assert.Error(t, appendRestArgs(fval, []string{"notafloat"}))
	})

	t.Run("float64", func(t *testing.T) {
		var s []float64
		fval := reflect.ValueOf(&s).Elem()
		assert.NotError(t, appendRestArgs(fval, []string{"1.1", "2.2"}))
		assert.True(t, slices.Equal(s, []float64{1.1, 2.2}))
	})
	...
	t.Run("unsupported type returns ErrInvalidSpecification", func(t *testing.T) {
		var s []chan int
		fval := reflect.ValueOf(&s).Elem()
		err := appendRestArgs(fval, []string{"x"})
		assert.Error(t, err)
		assert.ErrorIs(t, err, ErrInvalidSpecification)
	})
}
