...
// functions. Use with iterators to build worker pools.
...
// ContextChannelWorker creates a worker function that wraps a context
// and will--when called--block until the context is done, returning
// the context's cancellation error. Unless provided with a custom
// context that can be canceled but does not return an error (which
// would break many common assumptions regarding contexts,) this
// worker will always return an error.
