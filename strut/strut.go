...
// ExtendParagraph writes each string from the iterator 'seq' on its own
// line to the builder, then adds an additional blank line at the end
// to form a paragraph.
func (b *Builder) ExtendParagraph(seq iter.Seq[string]) { b.ExtendLines(seq); b.Line() }
