...
// LockedMap provides a map implementation that maintains insertion order.
...
func (m *LockedMap[K, V]) init()              { m.once.Do(m.doInit) }
func (m *LockedMap[K, V]) doInit()            { m.hash = stw.Map[K, V]{} }
func (*LockedMap[K, V]) zerov() (out V)       { return }
func (*LockedMap[K, V]) with(mtx *sync.Mutex) { mtx.Unlock() }
func (m *LockedMap[K, V]) lock() *sync.Mutex  { m.mtx.Lock(); m.init(); return &m.mtx }

// Len returns the length of the map.
func (m *LockedMap[K, V]) Len() int { defer m.with(m.lock()); return m.hash.Len() }

// Check returns true if the value K is in the map.
func (m *LockedMap[K, V]) Check(key K) bool { defer m.with(m.lock()); return m.hash.Check(key) }

// Get returns the value from the map. If the key is not present in the map,
// this returns the zero value for V.
func (m *LockedMap[K, V]) Get(key K) V { out, _ := m.Load(key); return out }

// Load returns the value in the map for the key, and an "ok" value
// which is true if that item is present in the map.
func (m *LockedMap[K, V]) Load(key K) (V, bool) { defer m.with(m.lock()); return m.hash.Load(key) }

// Delete removes a key from the map.
func (m *LockedMap[K, V]) Delete(k K) { defer m.with(m.lock()); m.hash.Delete(k) }
...
func (m *LockedMap[K, V]) Set(key K, value V) bool {
	defer m.with(m.lock())
...
func (m *LockedMap[K, V]) Ensure(key K) bool {
	defer m.with(m.lock())
	if m.hash.Check(key) {
		return true
	}
...
func (m *LockedMap[K, V]) Store(k K, v V) { m.Set(k, v) }

// Extend adds a sequence of key-value pairs to the map.
func (m *LockedMap[K, V]) Extend(seq iter.Seq2[K, V]) { irt.Apply2(seq, m.Store) }

// Iterator returns a standard Go iterator interface to the key-value
// pairs of the map in insertion order.
func (m *LockedMap[K, V]) Iterator() iter.Seq2[K, V] {
	m.init()
	return irt.WithMutex2(m.hash.Iterator(), &m.mtx)
}

// Keys provides an iterator over just the keys in the map in insertion order.
func (m *LockedMap[K, V]) Keys() iter.Seq[K] { return irt.WithMutex(m.hash.Keys(), &m.mtx) }

// Values provides an iterator over just the values in the map in insertion order.
func (m *LockedMap[K, V]) Values() iter.Seq[V] { return irt.WithMutex(m.hash.Values(), &m.mtx) }
...
func (m *LockedMap[K, V]) UnmarshalJSON(in []byte) error {
	m.init()
